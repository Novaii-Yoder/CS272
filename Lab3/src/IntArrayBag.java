...
public class IntArrayBag implements Cloneable
{
   ...
   **/   
   ...
   /**
    * Compares one bag to an object to see if they have the same data, and size.
    * @param obj
    *    the object you want test equality with
    * @precondition
    *    the obj parameter isn't null
    * @return
    *    A boolean telling if the bag and obj have the same data, and size.
    * @exception NullPointerException
    *    Indicates that obj is null.
    */
   public boolean equals(Object obj) {
      if (obj instanceof IntArrayBag) {
         IntArrayBag candidate = (IntArrayBag) obj;

         if (manyItems != candidate.manyItems)
            return false;

         else
            for (int i = 0; i < manyItems; i++)
               if (countOccurrences(data[i]) != candidate.countOccurrences(candidate.data[i]))
                  return false;
            return true;
      } // end of if
      else
   ...
   /**
    * Removes all elements of removed from this bag.
    * @param removed
    *    Pointer to the bag that will be removed from this bag.
    * @precondition
    *    Removed bag is not null
    * @return changed
    *    A boolean, false if no changes to this bag were made, otherwise true
    * @exception NullPointerException
    *    Indicates that removed is null.
    */
   public boolean removeAll (IntArrayBag removed) {
      boolean changed = false;
      for (int i: removed.data)
         if(remove(i))
            changed = true;
      return changed;
   } // end of removeAll
   ...
   /**
    * Creates a new bag filled with the elements that are found in both b1 and b2.
    * @param b1
    *    The first of two bags.
    * @param b2
    *    The second of two bags.
    * @precondition
    *    That neither of the bags are null.
    * @return b3
    *    A new bag with the values found in both b1 and b2.
    * @exception NullPointerException
    *     Indicates that removed is null.
    */
   public static IntArrayBag intersection(IntArrayBag b1, IntArrayBag b2) {
      IntArrayBag b3 = new IntArrayBag();
      if (b1.manyItems == 0 || b2.manyItems == 0)
         return b3;

      for (int i: b1.data) {
         int numOccur1 = b1.countOccurrences(i);
         int numOccur2 = b2.countOccurrences(i);
         int numOccur3 = b3.countOccurrences(i);
         if (numOccur1 != 0 && numOccur2 != 0) {
            if (numOccur1 <= numOccur2 && numOccur1 > numOccur3)
               for (int j = 0; j < numOccur1 - numOccur3; j++)
                  b3.add(i);
            else if (numOccur1 > numOccur2 && numOccur2 > numOccur3)
               for (int j = 0; j < numOccur2 - numOccur3; j++)
                  b3.add(i);
         } // end of if
      } // end of inhanced for
      return b3;
   ...
   /**
    * Converts all the data to a string that can be used to show the data
    * all at once. In the for {data[0], data[1]...}, returns {} if bag is empty.
    * @return
    *    A string with all the data enclosed by {}, returns just {} if bag is empty.
    */
